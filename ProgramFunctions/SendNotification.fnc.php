 ...
 * @param string $to         To email address. Defaults to $BETHELNotifyAddress (see config.inc.php).
 *
 * @return bool  False if email not sent, else true.
 */
function SendNotificationCreateStudentAccount( $student_id, $to = '' )
{
	global $BETHELNotifyAddress;

	require_once 'ProgramFunctions/SendEmail.fnc.php';

	if ( empty( $to ) )
	{
		$to = $BETHELNotifyAddress;
	}
 ...
 * Do not send notification if BETHELSIS installed on localhost (Windows typically).
 *
 * @since 5.9
 *
 * @uses _BETHELLoginURL() function
 *
 * @param int    $student_id Student ID.
...
function SendNotificationActivateStudentAccount( $student_id, $to = '' )
{
	...
	$BETHEL_url = _BETHELLoginURL();

	if ( ( strpos( $BETHEL_url, '127.0.0.1' ) !== false
			|| strpos( $BETHEL_url, 'localhost' ) !== false )
		&& ! BETHEL_DEBUG )
	{
		// Do not send notification if BETHELSIS installed on localhost (Windows typically).
		return false;
	...
	$message = sprintf( $message, $student_id, $BETHEL_url );
...
function SendNotificationActivateUserAccount( $staff_id, $to = '' )
{
	...
	$message = sprintf( $message, $staff_id, $BETHEL_url );
 ...
 * Do not send notification if password not set.
 * Send notification even if BETHELSIS installed on localhost (Windows typically)
