...
if ( ! function_exists( 'FirstLoginPoll' ) )
{
	 ...
	 *
	 * @return string Poll HTML array or empty string if not 'admin' user or if BETHELsis.org not reachable.
	 */
	function FirstLoginPoll()
	{
		...
		$has_connection = @file_get_contents( '#' );
		...
		$fields[] = '<input type="hidden" name="version" value="' . BETHEL_VERSION . '" />';
		...
		$fields[] = sprintf(
			...
			URLEscape( 'https://www.BETHELsis.org/' . $url_lang . 'installation-poll/' )
		);
		...
		$form = '<form action="https://www.BETHELsis.org/installation-poll/poll-submit.php" method="POST" id="first-login-poll-form" target="_top">';
