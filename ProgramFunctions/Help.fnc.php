...
function HelpLoad( $force = false )
{
	...
	$non_core_modules = array_diff( array_keys( $BETHELModules ), $BETHELCoreModules );
 ...
 * - Replace 'BETHELSIS' with configured app name.
 *
 * @since 4.3
 *
 * @uses GetHelpTextRaw()
...
function GetHelpText( $modname )
{
	...
	// Replace BETHELSIS with configured app name.
	$help_text = str_replace( 'BETHELSIS', Config( 'NAME' ), $help_text );
...
function GetHelpTextRaw( $modname )
{
	...
	{
		// FJ fix bug URL Modules.php?modname=Student_Billing/Statements.php&_BETHEL_PDF.
