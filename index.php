	...
	&& $_REQUEST['PASSWORD'] !== '' )
{
	...
	elseif ( isset( $_COOKIE['BETHELSIS'] ) )
	{
	...
	{
		// First Login.
		header( 'Location: index.php?locale=' . $_SESSION['locale'] . 'Modules.php?modname=misc/Portal.php' );
...
?>
	...
	<?php // Choose language.
	if ( count( $BETHELLocales ) > 1 ) : ?>

		<tr>
			<td>
			<?php foreach ( $BETHELLocales as $loc ) :
	...
	// @link #s
	echo _( 'About' ); ?></label>
	<div class="about-BETHELsis toggle-me">
		...
		<p class="center">
			<?php echo sprintf( _( '%s version %s' ), 'Bethel Portal System', 1.0 ); ?>
		</p>
		<p class="center size-1">
			&copy; 
			<br />&copy;2021 <a href="#" noreferrer>Bethel Academy Offical Website</a>
		</p>
	</div>
...
elseif ( ! isset( $_REQUEST['create_account'] ) )
{
	...
	$redirect_to = empty( $_REQUEST['redirect_to'] ) ?
		...
		str_replace(
			array( '&_BETHEL_PDF=true', '&_BETHEL_PDF', '&LO_save=1' ),
			'',
