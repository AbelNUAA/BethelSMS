	...
	&& $config_table_exists )
{
	...
	// Translate Database.
	if ( $_POST['lang'] === 'fr'
		&& file_exists( 'BETHELsis_fr.sql' ) )
	{
		$BETHELsis_sql = file_get_contents( 'BETHELsis_fr.sql' );
	}
	elseif ( $_POST['lang'] === 'es'
		&& file_exists( 'BETHELsis_es.sql' ) )
	{
		$BETHELsis_sql = file_get_contents( 'BETHELsis_es.sql' );
	}

	$result = db_query( $BETHELsis_sql );

	die( 'Success: database translated. <a href="index.php">Access BETHELSIS</a>' );
}

if ( ! file_exists( 'BETHELsis.sql' ) )
{
	die( 'Error: BETHELsis.sql file not found.' );
}

$BETHELsis_sql = file_get_contents( 'BETHELsis.sql' );

$result = db_query( $BETHELsis_sql );

if ( file_exists( 'BETHELsis_addons.sql' ) )
{
	// @since 5.1 Install add-ons.
	$sql_addons = file_get_contents( 'BETHELsis_addons.sql' );
...
die( 'Success: database' .
	( file_exists( 'BETHELsis_addons.sql' ) ? ' and add-ons' : '' ) .
	' installed. <a href="index.php">Access BETHELSIS</a>' );
