...
<?php // Generate Menu.
	...
	$menu_key = array_keys( $_BETHEL['Menu'] );
	...
	for ( $i = 0; $i < $size_menu; $i++ ) :
		...
		if ( count( $modcat_menu = $_BETHEL['Menu'][ $menu_i ] ) ) :
		...
		<li class="menu-module <?php echo $modcat_class; ?>">
			<a href="<?php echo URLEscape( 'Modules.php?modname=' . $modcat_menu['default'] ); ?>" class="menu-top">
				...
				<?php if ( ! in_array( $menu_i, $BETHELCoreModules ) ) :
			...
			for ( $j = 0; $j < $size_modcat; $j++ )
			{
				...
				$title = $_BETHEL['Menu'][ $menu_i ][ $modcat_j ];
				...
				if ( ! is_numeric( $modcat_j ) ) :
					...
					$target = ( mb_strpos( $modcat_j, '_BETHEL_PDF' ) !== false ?
						' target="_blank"' :
						''
					); ?>
