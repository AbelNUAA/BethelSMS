 ...
 * Compatibility with old sequence names before BETHELSIS 5.0.
 * Should be updated
 * @see _update50beta()
 *
 * @since 5.0
 ...
 * Send notification email if `$BETHELNotifyAddress` or `$BETHELErrorsAddress` set
 *
 * @global string $BETHELNotifyAddress or $BETHELErrorsAddress email set in config.inc.php file
...
function db_show_error( $sql, $failnote, $additional = '' )
{
	...
	// Send notification email if $BETHELNotifyAddress set & functions loaded.
	$db_error_email = ! empty( $BETHELErrorsAddress ) ? $BETHELErrorsAddress : $BETHELNotifyAddress;
