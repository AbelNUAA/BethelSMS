...
/***************STUDENTS**/
		...
		case 'Students/Student.php|header':
					...
					|| $_REQUEST['category_id'] == 1 ) ) // General Info.
			{
				...
				if ( UserStudentID() )
				{
					$old_student_in_moodle = (bool) MoodleXBETHELGet( 'student_id', UserStudentID() );
				}
		...
		case 'Students/Student.php|create_student_checks':
			...
			if ( ! empty( $_REQUEST['moodle_create_student'] )
				&& ( empty( $_REQUEST['students']['USERNAME'] )
					|| empty( $_REQUEST['students'][BETHEL_STUDENTS_EMAIL_FIELD] ) ) )
			{
...
/***************USERS**/
		...
		case 'Users/User.php|header':
					...
					|| $_REQUEST['category_id'] == 1 ) ) // General Info
			{
				...
				if ( UserStaffID() )
				{
					$old_user_in_moodle = (bool) MoodleXBETHELGet( 'staff_id', UserStaffID() );
				}
		...
		case 'Users/User.php|update_user':
			...
			elseif ( MoodleXBETHELGet( 'staff_id', UserStaffID() ) )
			{
				Moodle( $modname, 'core_user_update_users' );
				Moodle( $modname, 'core_role_unassign_roles' );
				Moodle( $modname, 'core_role_assign_roles' );
...
/***************SCHEDULING**/
		...
		case 'Scheduling/Courses.php|header':
			...
			if ( AllowEdit() )
			{
				...
				{
					$old_course_period_in_moodle = (bool) MoodleXBETHELGet( 'course_period_id', $_REQUEST['course_period_id'] );
				}
				...
				{
					$course_in_moodle = (bool) MoodleXBETHELGet( 'course_id', $_REQUEST['course_id'] );
				}
		...
		case 'Scheduling/Courses.php|update_course_period':
			...
			if ( MoodleXBETHELGet( 'course_period_id', $_REQUEST['course_period_id'] ) )
			{
...
/***************SCHOOL_SETUP**/
		...
		case 'School_Setup/Calendar.php|update_calendar_event':
			...
			$is_moodle_event = (bool) MoodleXBETHELGet( 'calendar_event_id', $_REQUEST['event_id'] );
		...
		case 'School_Setup/Rollover.php|rollover_checks':
			...
			{
				//SUBJECT_ID
				DBQuery( "UPDATE MOODLEXBETHEL SET BETHEL_ID=(SELECT ROLLOVER_ID FROM COURSE_SUBJECTS WHERE SUBJECT_ID=BETHEL_ID) WHERE exists(SELECT * FROM COURSE_SUBJECTS WHERE SUBJECT_ID=BETHEL_ID AND ROLLOVER_ID IS NOT NULL AND SYEAR='" . $next_syear . "') AND \"column\"='subject_id'" );

				//COURSE_ID
				DBQuery( "UPDATE MOODLEXBETHEL SET BETHEL_ID=(SELECT ROLLOVER_ID FROM COURSES WHERE COURSE_ID=BETHEL_ID) WHERE exists(SELECT * FROM COURSES WHERE COURSE_ID=BETHEL_ID AND ROLLOVER_ID IS NOT NULL AND SYEAR='" . $next_syear . "') AND \"column\"='course_id'" );

				//COURSE_PERIOD_ID
				$course_periods_RET = DBGet( "SELECT mxc.MOODLE_ID AS CP_MOODLE_ID, cp.TEACHER_ID FROM COURSE_PERIODS cp, MOODLEXBETHEL mxc WHERE cp.SYEAR='" . $next_syear . "' AND cp.SCHOOL_ID='" . UserSchool() . "' AND cp.ROLLOVER_ID IS NOT NULL AND cp.ROLLOVER_ID=mxc.BETHEL_ID AND mxc.\"column\"='course_period_id'" );
			...
			{
				DBQuery( "UPDATE MOODLEXBETHEL SET BETHEL_ID=(SELECT ROLLOVER_ID FROM STAFF WHERE STAFF_ID=BETHEL_ID) WHERE exists(SELECT * FROM STAFF WHERE STAFF_ID=BETHEL_ID AND ROLLOVER_ID IS NOT NULL AND SYEAR='" . $next_syear . "') AND \"column\"='staff_id'" );
			}
		...
		case 'School_Setup/Rollover.php|rollover_after':
			...
			if ( ! empty( $_REQUEST['tables']['STAFF'] ) )
			{
				...
				{
					DBQuery( "UPDATE MOODLEXBETHEL SET BETHEL_ID='" . $value['STAFF_ID'] . "' WHERE BETHEL_ID='" . $value['ROLLOVER_ID'] . "' AND \"column\"='staff_id'" );
				}
			}
			...
			if ( ! empty( $_REQUEST['tables']['COURSES'] ) )
			{
				...
				{
					DBQuery( "UPDATE MOODLEXBETHEL SET BETHEL_ID='" . $value['SUBJECT_ID'] . "' WHERE BETHEL_ID='" . $value['ROLLOVER_ID'] . "' AND \"column\"='subject_id'" );
				}
				...
				{
					DBQuery( "UPDATE MOODLEXBETHEL
						SET BETHEL_ID='" . $value['COURSE_ID'] . "'
						WHERE BETHEL_ID='" . $value['ROLLOVER_ID'] . "'
				...
				$course_periods_RET = DBGet( "SELECT cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.SHORT_NAME,cp.MARKING_PERIOD_ID,cp.TEACHER_ID
					FROM COURSE_PERIODS cp,MOODLEXBETHEL mxc
					WHERE cp.SYEAR='" . $next_syear . "'
					AND cp.SCHOOL_ID='" . UserSchool() . "'
					AND cp.ROLLOVER_ID IS NOT NULL
					AND cp.ROLLOVER_ID=mxc.BETHEL_ID
					AND mxc.\"column\"='course_period_id'" );
...
/**
 * Get Moodle ID by BETHELSIS ID
 *
 * @since 6.0
 *
 * @uses MOODLEXBETHEL DB cross table.
 *
 * @param string $column     Column, what type of object.
 * @param int    $BETHEL_id BETHELSIS ID.
 *
 * @return int 0 or Moodle ID.
 */
function MoodleXBETHELGet( $column, $BETHEL_id )
{
	if ( ! $column
		|| ! $BETHEL_id )
	{
