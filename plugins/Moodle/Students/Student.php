...
function core_user_create_users_object()
{
	...
	$email = issetVal( $_REQUEST['students'][BETHEL_STUDENTS_EMAIL_FIELD] );
	$auth = 'manual';
...
function core_user_update_users_object()
{
	...
	$moodle_id = MoodleXBETHELGet( 'student_id', UserStudentID() );
	...
	$email = issetVal( $_REQUEST['students'][BETHEL_STUDENTS_EMAIL_FIELD], false );
...
function core_role_assign_roles_object()
{
	...
	$userid = (int) DBGetOne( "SELECT moodle_id
		FROM moodlexBETHEL
		WHERE BETHEL_id=(SELECT STAFF_ID FROM STUDENTS_JOIN_USERS
	...
	$studentid = MoodleXBETHELGet( 'student_id', $student_id );
...
//For the moment, component = user && filearea = private is hardcoded...
	...
	$BETHEL_id = $_POST['userId'];
	//gather the Moodle user ID
	$column = ( mb_strpos( $_POST['modname'], 'Users' ) !== false ? 'staff_id' : 'student_id' );
	$instanceid = MoodleXBETHELGet( $column, $BETHEL_id );
	...
	$filecontent = base64_encode_file(
		$BETHELPath . $_POST['photoPath'] . $_POST['sYear'] . '/' . $_POST['userId'] . '.jpg'
	);
