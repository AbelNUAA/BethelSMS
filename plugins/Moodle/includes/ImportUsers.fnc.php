...
/**
 * Filter Moodle Users
 * Users where confirmed=true, suspended=false, and id not exists in MOODLEXBETHEL table.
 *
 * @since 5.9
 *
...
function MoodleUsersFilter( $users )
{
	...
	$moodlexBETHEL_user_ids_RET = DBGet( "SELECT MOODLE_ID
		FROM MOODLEXBETHEL
		WHERE " . DBEscapeIdentifier( 'COLUMN' ) . "='student_id'
		OR " . DBEscapeIdentifier( 'COLUMN' ) . "='staff_id'", array(), array( 'MOODLE_ID' ) );

	$moodlexBETHEL_user_ids = array_keys( $moodlexBETHEL_user_ids_RET );

	foreach ( $users as $user )
	{
		if ( empty( $user['confirmed'] )
			|| ! empty( $user['suspended'] )
			|| in_array( $user['id'], $moodlexBETHEL_user_ids ) )
		{
			// User is not confirmed, or suspended, or already in BETHELSIS.
			continue;
		}
...
/**
 * Moake Moodle User Profile: select target user profile in BETHELSIS.
 *
 * @since 5.9
 *
