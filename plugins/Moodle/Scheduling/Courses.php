...
function core_course_create_categories_response( $response )
{
	...
	//then, save the ID in the moodlexBETHEL cross-reference table:
	/*
	list of (
	object {
	...
	{
		$column = 'subject_id';
		$BETHEL_id = $_REQUEST['subject_id'];
	}
	elseif ( $table_name == 'COURSES' )
	{
		$column = 'course_id';
		$BETHEL_id = (string) $_REQUEST['course_id'];
	}

	DBQuery( "INSERT INTO MOODLEXBETHEL (\"column\", BETHEL_id, moodle_id)
		VALUES('" . $column . "', '" . $BETHEL_id . "', " . $response[0]['id'] . ")" );
...
function core_course_update_categories_object()
{
	...
	$id = MoodleXBETHELGet( $column, $BETHEL_id );
...
function core_course_delete_categories_response( $response )
{
	...
	//delete the reference the moodlexBETHEL cross-reference table:
	DBQuery( "DELETE FROM MOODLEXBETHEL
		WHERE \"column\"='" . $column . "'
		AND BETHEL_id='" . $BETHEL_id . "'" );
...
function core_course_create_courses_object()
{
	...
	$categoryid = MoodleXBETHELGet( 'course_id', $_REQUEST['course_id'] );
...
function core_role_assign_roles_object()
{
	...
	$userid = MoodleXBETHELGet( 'staff_id', $columns['TEACHER_ID'] );
	...
	$courseid = MoodleXBETHELGet( 'course_period_id', $_REQUEST['course_period_id'] );
...
{
	//gather the Moodle course ID
	$id = MoodleXBETHELGet( 'course_period_id', $_REQUEST['course_period_id'] );
...
function core_course_delete_courses_response( $response )
{
	...
	DBQuery( "DELETE FROM MOODLEXBETHEL
		...
		AND BETHEL_id='" . $_REQUEST['course_period_id'] . "'" );
...
function core_role_unassign_roles_object()
{
	...
	$userid = MoodleXBETHELGet( 'staff_id', $current[1]['TEACHER_ID'] );
	...
	$courseperiodid = MoodleXBETHELGet( 'course_period_id', $_REQUEST['course_period_id'] );
...
function core_course_update_courses_object()
{
	...
	$moodle_id = MoodleXBETHELGet( 'course_period_id', $_REQUEST['course_period_id'] );
