...
function core_calendar_create_calendar_events_object()
{
	...
	$courseid = MoodleXBETHELGet( 'course_period_id', UserCoursePeriod() );
...
function core_calendar_delete_calendar_events_object()
{
	...
	$eventid = MoodleXBETHELGet( 'assignment_id', $assignment_id );
...
function core_calendar_delete_calendar_events_response( $response )
{
	...
	DBQuery( "DELETE FROM MOODLEXBETHEL
		WHERE \"column\"='assignment_id'
		AND BETHEL_id='" . $assignment_id . "'" );
