...
{
	$error[] = 'BETHELSIS requires PHP 5.4.45 to run, your version is : ' . PHP_VERSION;
}
...
{
	$error[] = 'PHP extensions: BETHELSIS relies on the pgsql (PostgreSQL) extension. Please install and activate it.';
}
...
{
	$error[] = 'The diagnostic.php file needs to be in the BETHELSIS directory to be able to run. Please move it there, and run it again.';
}
else
{
	require_once './Warehouse.php';

	if ( ! @opendir( $BETHELPath . '/functions' ) )
	{
		$error[] = 'The value for $BETHELPath in the config.inc.php file is not correct or else the functions directory does not have the correct permissions to be read by the webserver. Make sure $BETHELPath points to the BETHELSIS installation directory and that it is readable by all users.';
	}

	if ( ! function_exists( 'pg_connect' ) )
	{
		$error[] = 'The pgsql extension (see the php.ini file) is not activated OR PHP was not compiled with PostgreSQL support. You may need to recompile PHP using the --with-pgsql option for BETHELSIS to work.';
	}
	else
	{
		/**
		...
		if ( ! $connection )
		{
			$error[] = 'BETHELSIS cannot connect to the PostgreSQL database. Either Postgres is not running, it was not started with the -i option, or connections from this host are not allowed in the pg_hba.conf file. Last Postgres Error: ' . pg_last_error();
		}
		else
		{
			...
			if ( $result === false )
			{
				...
				{
					$error[] = 'The database was created with the wrong permissions. The user specified in the config.inc.php file does not have permission to access the BETHEL database. Use the super-user (postgres) or recreate the database adding \connect - YOUR_USERNAME to the top of the BETHELsis.sql file.';
				}
				elseif ( mb_strpos( $errstring, 'elation "config" does not exist' ) !== false )
				{
					$error[] = 'At least one of the tables does not exist. Make sure you ran the BETHELsis.sql file as described in the INSTALL.md file.';
				}
				elseif ( $errstring )
				{
					$error[] = $errstring;
			...
			if ( ! is_array( $BETHELLocales )
				|| empty( $BETHELLocales ) )
			{
				$error[] = 'The value for $BETHELLocales in the config.inc.php file is not correct.';
			}
		}
	}
...
{
	$error[] = 'PHP extensions: BETHELSIS relies on the gd extension (used to resize and compress images). Please install and activate it.';
}

// Check for zip extension.
if ( ! extension_loaded( 'zip' ) )
{
	$error[] = 'PHP extensions: BETHELSIS relies on the zip extension (used to upload add-ons and by Import add-ons). Please install and activate it.';
}
...
{
	$error[] = 'PHP extensions: BETHELSIS relies on the xmlrpc extension (only used to connect to Moodle). Please install and activate it.';
}

// Check for curl extension.
if ( ! extension_loaded( 'curl' ) )
{
	$error[] = 'PHP extensions: BETHELSIS relies on the curl extension (only used to connect to Moodle). Please install and activate it.';
}
...
if ( ! count( $error ) )
{
	echo '<h3>Your BETHELSIS installation is properly configured.</h3>';
}
