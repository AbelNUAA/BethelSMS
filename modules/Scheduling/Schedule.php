	...
	&& ! $_REQUEST['modfunc'] )
{
	...
	$print_schedules_link = 'Modules.php?modname=Scheduling/PrintSchedules.php&modfunc=save&st_arr[]=' .
		UserStudentID() . '&_BETHEL_PDF=true&schedule_table=Yes';
	?>
	<script>
...
{
	global $_BETHEL, $THIS_RET;

	$fy_id = GetFullYearMP();

	if ( empty( $_BETHEL['_makeMPSelect'] ) )
	{
		...
		$_BETHEL['_makeMPSelect'][$fy_id][1] = array(
		...
		{
			$_BETHEL['_makeMPSelect'][$fy_id][] = $sem;
		}

		foreach ( (array) $quarters_RET as $qtr )
		{
			$_BETHEL['_makeMPSelect'][$fy_id][] = $qtr;
		}
		...
		foreach ( (array) $quarters_RET as $qtr )
		{
			$_BETHEL['_makeMPSelect'][$qtr['MARKING_PERIOD_ID']][] = $qtr;
		}
	}

	if ( is_array( $_BETHEL['_makeMPSelect'][$mp_id] ) )
	{
		foreach ( (array) $_BETHEL['_makeMPSelect'][$mp_id] as $value )
		{
