...
function SendMessage( $msg )
{
	...
	// Sanitize Message.
	if ( version_compare( BETHEL_VERSION, '2.9-alpha', '>=' ) )
	{
		// Is MarkDown.
	...
	$msg_id = DBSeqNextID( 'messages_message_id_seq' );
...
function _getMessageRecipients( $recipients_key, $recipients_ids )
{
	...
	{
		$names_RET = DBGet( "SELECT array_agg(FIRST_NAME||' '||LAST_NAME||coalesce(' '||NAME_SUFFIX,' ')) AS NAMES
			FROM STUDENTS
			WHERE STUDENT_ID IN('" . implode( "','", $recipients_ids ) . "')" );
	}
	elseif ( $recipients_key === 'staff_id' )
	{
		$names_RET = DBGet( "SELECT array_agg(FIRST_NAME||' '||LAST_NAME) AS NAMES
			FROM STAFF
...
function _getAllowedAdminsRecipients()
{
	...
	if ( ! $allowed_ids )
	{
		$allowed_ids_RET = DBGet( "SELECT array_agg(STAFF_ID) as ALLOWED_IDS
			FROM STAFF
			WHERE PROFILE='admin'
			AND SYEAR='" . UserSyear() . "'
...
function _getStudentAllowedTeachersRecipients( $student_id )
{
	...
	{
		$allowed_ids_RET = DBGet( "SELECT array_agg(DISTINCT(cp.TEACHER_ID)) as ALLOWED_IDS
...
function _getTeacherAllowedParentsRecipients()
{
	...
	if ( ! $allowed_ids )
	{
		$allowed_ids_RET = DBGet( "SELECT array_agg(sju.STAFF_ID) as ALLOWED_IDS
...
function GetReplySubjectMessage( $msg_id )
{
	...
	return array( 'subject' => $subject, 'message' => $message, 'to' => $to );
}
