		...
		case 'honor_roll_subject':
			...
			if ( ! empty( $_REQUEST['subject_id'] ) )
			{
				...
				if ( ! $extra['NoSearchTerms'] )
				{
					...
					$_BETHEL['SearchTerms'] .= '<b>' . _( 'Honor Roll by Subject' ) . ':</b> ' .
						$subject_RET[1]['TITLE'];

					$_BETHEL['SearchTerms'] .= '<input type="hidden" id="subject_id" name="subject_id" value="' .
				...
				&& ! empty( $_REQUEST['high_honor_roll'] ) )
			{
				...
				if ( ! $extra['NoSearchTerms'] )
				{
					$_BETHEL['SearchTerms'] .= '<b>' . _( 'Honor Roll' ) . ' & ' .
			...
			elseif ( ! empty( $_REQUEST['honor_roll'] ) )
			{
				...
				if ( ! $extra['NoSearchTerms'] )
				{
					$_BETHEL['SearchTerms'] .= '<b>' . _( 'Honor Roll' ) . '</b><br />';
				}
			}
			elseif ( ! empty( $_REQUEST['high_honor_roll'] ) )
			{
				...
				if ( ! $extra['NoSearchTerms'] )
				{
					$_BETHEL['SearchTerms'] .= '<b>' . _( 'High Honor Roll' ) . '</b><br />';
				}
			}
...
function HonorRollFrame( $frame_file = array() )
{
	...
	if ( $frame_file )
	{
		...
		if ( $error && isset( $_REQUEST['_BETHEL_PDF'] ) )
		{
