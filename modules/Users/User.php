		...
		|| ! empty( $_FILES ) )
	{
		...
		if ( UserStaffID() )
		{
			...
			if ( basename( $_SERVER['PHP_SELF'] ) === 'index.php' )
			{
				 ...
				 * @since 5.7
				 */
				SendNotificationCreateUserAccount( UserStaffID(), $BETHELNotifyAddress );
			}

			if ( ! empty( $staff_id ) ) // $staff_id is set on new.
			{
				// Send New Administrator Account email to System Notify address.
				SendNotificationNewAdministrator( UserStaffID(), $BETHELNotifyAddress );
			}
	...
	&& $_REQUEST['modfunc'] !== 'remove_file' )
{
		...
		|| ! empty( $_REQUEST['delete_ok'] ) )
	{
		...
		$_BETHEL['selected_tab'] = 'Modules.php?modname=' . $_REQUEST['modname'] . '&category_id=' . $category_id . '&staff_id=' . UserStaffID();
