...
else
{
	...
	if ( empty( $extra['NoSearchTerms'] ) )
	{
		...
		{
			$_BETHEL['SearchTerms'] .= '<b>' . _( 'Search All Schools' ) . '</b><br />';
		}

		if ( isset( $_REQUEST['include_inactive'] )
			&& $_REQUEST['include_inactive'] === 'Y' )
		{
			$_BETHEL['SearchTerms'] .= '<b>' . _( 'Include Inactive Students' ) . '</b><br />';
		}
	}
			...
			&& count( (array) $students_RET ) == 1 ) )
	{
		...
		if ( ! empty( $_BETHEL['SearchTerms'] ) )
		{
			DrawHeader( mb_substr( $_BETHEL['SearchTerms'], 0, -6 ) );
		}
