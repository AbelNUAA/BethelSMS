		...
		|| ! empty( $_FILES ) )
	{
		...
		if ( UserStudentID() )
		{
			...
			if ( basename( $_SERVER['PHP_SELF'] ) === 'index.php' )
			{
				 ...
				 * @since 5.7
				 */
				SendNotificationCreateStudentAccount( UserStudentID(), $BETHELNotifyAddress );
			}
	...
	&& $_REQUEST['modfunc'] !== 'remove_file' )
{
		...
		|| ! empty( $_REQUEST['delete_ok'] ) )
	{
		...
		$_BETHEL['selected_tab'] = 'Modules.php?modname=' . $_REQUEST['modname'] .
