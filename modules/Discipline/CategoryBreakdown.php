...
if ( ! $_REQUEST['modfunc'] )
{
	...
	if ( isset( $_BETHEL['SearchTerms'] )
		&& !empty( $_BETHEL['SearchTerms'] ) )
	{
		DrawHeader( $_BETHEL['SearchTerms'] );
	}
	...
	if ( ! empty( $_REQUEST['category_id'] ) )
	{
		...
		$_BETHEL['selected_tab'] = PreparePHP_SELF( $_REQUEST );
		...
		else
		{
			...
			{
				$search_terms = ' - ' . strip_tags( str_replace( '<br />', " - ", mb_substr( $_BETHEL['SearchTerms'], 0, -6 ) ));
			}
