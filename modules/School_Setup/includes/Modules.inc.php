...
// Core modules (packaged with BETHELSIS):
// Core modules cannot be deleted
/* var defined in Warehouse.php
$BETHELCoreModules = array(
'School_Setup',
'Students',
'Users',
'Scheduling',
	...
	&& AllowEdit() )
{
		...
		&& ( $addon_zip_path = FileUpload( 'upload', $FileUploadsPath, array( '.zip' ), FileUploadMaxSize(), $error ) ) )
	{
		...
		if ( $zip_open === true )
		{
			...
			if ( ! in_array( $addon_dir, $BETHELCoreModules ) )
			{
	...
	&& AllowEdit() )
{
	if ( DeletePrompt( _( 'Module' ) ) )
	{
		...
		// Verify if not in $always_activated & not in $BETHELCoreModules but in $BETHELModules.
		$can_delete = $can_delete
			&& ! in_array( $_REQUEST['module'], $always_activated )
			&& ! in_array( $_REQUEST['module'], $BETHELCoreModules )
			&& in_array( $_REQUEST['module'], array_keys( $BETHELModules ) )
			&& $BETHELModules[$_REQUEST['module']] == false;
		...
		if ( $can_delete )
		{
			...
			//update $BETHELModules
			unset( $BETHELModules[$_REQUEST['module']] );
	...
	&& AllowEdit() )
{
	if ( DeletePrompt( _( 'Module' ), _( 'Deactivate' ) ) )
	{
		...
		if ( ! in_array( $_REQUEST['module'], $always_activated ) && in_array( $_REQUEST['module'], array_keys( $BETHELModules ) ) && $BETHELModules[$_REQUEST['module']] == true )
		{
			//update $BETHELModules
			$BETHELModules[$_REQUEST['module']] = false;
...
if ( $_REQUEST['modfunc'] === 'activate'
	&& AllowEdit() )
{
	$update_BETHELModules = false;

	//verify not already in $BETHELModules

	if ( ! in_array( $_REQUEST['module'], array_keys( $BETHELModules ) ) )
	{
	...
	//verify in $BETHELModules
	elseif ( $BETHELModules[$_REQUEST['module']] == false && is_dir( 'modules/' . $_REQUEST['module'] ) )
	{
		$update_BETHELModules = true;
	}
	...
	if ( $update_BETHELModules )
	{
		//update $BETHELModules
		$BETHELModules[$_REQUEST['module']] = true;
...
if ( ! $_REQUEST['modfunc'] )
{
	...
	foreach ( (array) $BETHELModules as $module_title => $activated )
	{
		$THIS_RET = array();
...
{
	global $BETHELModules, $always_activated, $BETHELCoreModules;
	...
	else
	{
		if ( file_exists( 'modules/' . $module_title . '/Menu.php' ) )
		{
			...
			$can_delete = $can_delete
				...
				&& ! in_array( $module_title, $BETHELCoreModules )
				&& in_array( $module_title, array_keys( $BETHELModules ) );
...
{
	global $BETHELCoreModules;

	//format & translate module title

	if ( ! in_array( $module_title, $BETHELCoreModules ) && $activated )
	{
...
function _saveBETHELModules()
{
	global $BETHELModules;

	$MODULES = DBEscapeString( serialize( $BETHELModules ) );
