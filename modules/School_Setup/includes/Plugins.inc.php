...
// Core plugins (packaged with BETHELSIS):
// Core plugins cannot be deleted
/* var defined in Warehouse.php
$BETHELCorePlugins = array(
'Moodle'
);*/
...
if ( $_REQUEST['modfunc'] === 'config' )
{
	...
	if ( in_array( $_REQUEST['plugin'], array_keys( $BETHELPlugins ) ) && $BETHELPlugins[$_REQUEST['plugin']] == true && file_exists( 'plugins/' . $_REQUEST['plugin'] . '/config.inc.php' ) )
	{
	...
	&& AllowEdit() )
{
	if ( DeletePrompt( _( 'Plugin' ) ) )
	{
		...
		$can_delete = ! defined( 'BETHEL_DISABLE_ADDON_DELETE' ) || ! BETHEL_DISABLE_ADDON_DELETE;

		// Verify if not in $BETHELCorePlugins but in $BETHELPlugins.
		$can_delete = $can_delete
			&& ! in_array( $_REQUEST['plugin'], $BETHELCorePlugins )
			&& in_array( $_REQUEST['plugin'], array_keys( $BETHELPlugins ) )
			&& $BETHELPlugins[$_REQUEST['plugin']] == false;
		...
		if ( $can_delete )
		{
			...
			//update $BETHELPlugins
			unset( $BETHELPlugins[$_REQUEST['plugin']] );
	...
	&& AllowEdit() )
{
	...
	if ( ( ! defined( 'BETHEL_DISABLE_ADDON_UPLOAD' )
			|| ! BETHEL_DISABLE_ADDON_UPLOAD )
		...
		&& ( $addon_zip_path = FileUpload( 'upload', $FileUploadsPath, array( '.zip' ), FileUploadMaxSize(), $error ) ) )
	{
		...
		if ( $zip_open === true )
		{
			...
			if ( ! in_array( $addon_dir, $BETHELCorePlugins ) )
			{
	...
	&& AllowEdit() )
{
	if ( DeletePrompt( _( 'Plugin' ), _( 'Deactivate' ) ) )
	{
		...
		if ( in_array( $_REQUEST['plugin'], array_keys( $BETHELPlugins ) ) && $BETHELPlugins[$_REQUEST['plugin']] == true )
		{
			//update $BETHELPlugins
			$BETHELPlugins[$_REQUEST['plugin']] = false;
...
if ( $_REQUEST['modfunc'] === 'activate'
	&& AllowEdit() )
{
	$update_BETHELPlugins = false;

	//verify not already in $BETHELPlugins

	if ( ! in_array( $_REQUEST['plugin'], array_keys( $BETHELPlugins ) ) )
	{
	...
	//verify in $BETHELPlugins
	elseif ( $BETHELPlugins[$_REQUEST['plugin']] == false && is_dir( 'plugins/' . $_REQUEST['plugin'] ) )
	{
		$update_BETHELPlugins = true;
	}
	...
	if ( $update_BETHELPlugins )
	{
		//update $BETHELPlugins
		$BETHELPlugins[$_REQUEST['plugin']] = true;
...
if ( ! $_REQUEST['modfunc'] )
{
	...
	foreach ( (array) $BETHELPlugins as $plugin_title => $activated )
	{
		$THIS_RET = array();
...
{
	global $BETHELPlugins, $BETHELCorePlugins;
	...
	else
	{
		if ( file_exists( 'plugins/' . $plugin_title . '/functions.php' ) )
		{
			...
			$can_delete = $can_delete
				&& ! in_array( $plugin_title, $BETHELCorePlugins )
				&& in_array( $plugin_title, array_keys( $BETHELPlugins ) );
...
{
	global $BETHELCorePlugins;

	//format & translate plugin title

	if ( ! in_array( $plugin_title, $BETHELCorePlugins ) && $activated )
	{
...
function _saveBETHELPlugins()
{
	global $BETHELPlugins;

	$PLUGINS = DBEscapeString( serialize( $BETHELPlugins ) );
