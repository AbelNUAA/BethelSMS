...
function ListOutput( $result, $column_names, $singular = '.', $plural = '.', $link = array(), $group = array(), $options = array() )
{
			...
			&& ! isset( $_REQUEST['_BETHEL_PDF'] ) ) )
	{
			...
			&& $result_count > 0 )
		{
			...
			'&amp;_BETHEL_PDF=true" target="_blank"><img src="assets/themes/' .
	...
	if ( $result_count > 0 )
	{
		echo '<div class="list-wrapper"><table class="list widefat' .
			( $options['responsive'] && ! isset( $_REQUEST['_BETHEL_PDF'] ) ? ' rt' : '' ) .
		...
		{
			if ( $link['add']['link'] && ! isset( $_REQUEST['_BETHEL_PDF'] ) )
			{
				echo '<tr><td colspan="' . ( $remove ? $cols + 1 : $cols ) . '">' .
					button( 'add', $link['add']['title'], $link['add']['link'] ) . '</td></tr>';
			}
			elseif ( $link['add']['span'] && ! isset( $_REQUEST['_BETHEL_PDF'] ) )
			{
				echo '<tr><td colspan="' . ( $remove ? $cols + 1 : $cols ) . '">' .
			...
			elseif ( $link['add']['html'] && $cols )
			{
				...
				if ( $remove && ! isset( $_REQUEST['_BETHEL_PDF'] ) && $link['add']['html']['remove'] )
				{
					echo '<td>' . $link['add']['html']['remove'] . '</td>';
				}
				elseif ( $remove && ! isset( $_REQUEST['_BETHEL_PDF'] ) )
				{
		...
		for ( $i = $start; $i <= $stop; $i++ )
		{
			...
			if ( isset( $_REQUEST['_BETHEL_PDF'] ) && count( $item ) )
			{
				$key = array_keys( $item );
				$size = count( $key );
			...
			if ( $remove && ! isset( $_REQUEST['_BETHEL_PDF'] ) )
			{
			...
			if ( $cols )
			{
				foreach ( (array) $column_names as $key => $value )
				{
					...
					if ( empty( $link[$key] ) || $item[$key] === false || isset( $_REQUEST['_BETHEL_PDF'] ) )
					{
		...
		if ( ! isset( $link['add']['first'] )
			|| ( $stop - $start + 1 ) < $link['add']['first'] )
		{
			if ( isset( $link['add']['link'] ) && ! isset( $_REQUEST['_BETHEL_PDF'] ) )
			{
				echo '<tr><td colspan="' . ( $remove ? $cols + 1 : $cols ) . '">' .
				button(
					'add',
			...
			elseif ( isset( $link['add']['span'] ) && ! isset( $_REQUEST['_BETHEL_PDF'] ) )
			{
				echo '<tr><td colspan="' . ( $remove ? $cols + 1 : $cols ) . '">' .
					button( 'add' ) . $link['add']['span'] . '</td></tr>';
			}
...
class BETHEL_List implements Countable
{
	/**
