 ...
 *
 * @global $_BETHEL Used in Search.inc.php
 *
 ...
 * Also sets $_BETHEL['SearchTerms'] to display search term
 *
 * @since 3.0
 *
 ...
 *
 * @global array  $_BETHEL Sets $_BETHEL['SearchTerms']
 *
...
function SearchField( $field, $type = 'student', $extra = array() )
{
	...
	if ( ! $no_search_terms )
	{
		$_BETHEL['SearchTerms'] .= '<b>' . $field['TITLE'] . ':</b> ';
	}
	...
	switch ( $field['TYPE'] )
	{
		...
		case 'text':
			...
			if ( $value === '!' )
			{
				if ( ! $no_search_terms )
				{
					$_BETHEL['SearchTerms'] .= _( 'No Value' ) . '<br />';
				}
				...
				&& mb_substr( $value, -1 ) === '"' )
			{
				if ( ! $no_search_terms )
				{
					$_BETHEL['SearchTerms'] .= mb_substr( $value, 1, -1 ) . '<br />';
				}
			...
			if ( ! $no_search_terms )
			{
				$_BETHEL['SearchTerms'] .= _( 'starts with' ) . ' ' .
		...
		case 'date':
			...
			if ( ! $no_search_terms )
			{
				$_BETHEL['SearchTerms'] .= '<span class="sizep2">' . $part['html'] . '</span> ';

				if ( $field['TYPE'] === 'date' )
				{
					$_BETHEL['SearchTerms'] .= ProperDate( $value );
				}
				else
					$_BETHEL['SearchTerms'] .= $value;
		...
		case 'autos':
				...
				&& $value === '~' )
			{
				if ( ! $no_search_terms )
				{
					$_BETHEL['SearchTerms'] .= _( 'Other Value' ) . '<br />';
				}
