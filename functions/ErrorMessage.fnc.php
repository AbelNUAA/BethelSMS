...
/**
 * Send Error by Email
 * Send email to $BETHELErrorsAddress if set {@see config.inc.php}.
 *
 * @since 4.0
...
function ErrorSendEmail( $error = array(), $title = 'PHP Fatal error' )
{
	...
	if ( ! filter_var( $BETHELErrorsAddress, FILTER_VALIDATE_EMAIL ) )
	{
		return false;
	}
	...
	SendEmail( $BETHELErrorsAddress, $title, $message );
