...
function GetStuList( &$extra = array() )
{
	...
	if ( ! isset( $_REQUEST['_BETHEL_PDF'] )
		&& BETHEL_DEBUG ) // Activate only for debug purpose.
	{
		echo '<!--' . $sql . '-->';
	}
 ...
 * @global $_BETHEL       checks $_BETHEL['makeParents']
 *
 * @param  string $student_id Student ID.
	...
	if ( $_BETHEL['makeParents'] )
	{
		...
		if ( $_BETHEL['makeParents'] != '!' )
		{
			$constraint = " AND (lower(sjp.STUDENT_RELATION) LIKE '" .
				mb_strtolower( $_BETHEL['makeParents'] ) . "%')";
		}
	}
 ...
 *
 * @global $_BETHEL sets $_BETHEL['SearchTerms']
 *
 * @uses SearchField()
 *
...
function appendSQL( $sql, $extra = array() )
{
	...
	$_BETHEL['SearchTerms'] = issetVal( $_BETHEL['SearchTerms'], '' );
		...
		&& ! empty( $_REQUEST['stuid'] ) )
	{
		...
		if ( $stuids )
		{
			...
			if ( ! $no_search_terms )
			{
				$_BETHEL['SearchTerms'] .= '<b>' . sprintf( _( '%s ID' ), Config( 'NAME' ) ) .
					': </b>' . $stuids . '<br />';
			}
		}
	}
		...
		&& $_REQUEST['grade'] > 0 )
	{
		...
		if ( ! $no_search_terms )
		{
			$_BETHEL['SearchTerms'] .= '<b>' . _( 'Grade Level' ) . ': </b>' .
		...
		&& count( $_REQUEST['grades'] ) )
	{
		...
		if ( ! $no_search_terms )
		{
			$_BETHEL['SearchTerms'] .= '<b>' . ngettext( 'Grade Level', 'Grade Levels', count( $_REQUEST['grades'] ) ) .
		...
		foreach ( (array) $_REQUEST['grades'] as $grade_id => $y )
		{
			...
			if ( ! $no_search_terms )
			{
				$_BETHEL['SearchTerms'] .= $sep . GetGrade( $grade_id );
			}
	...
	if ( ! empty( $_REQUEST['addr'] ) )
	{
		...
		if ( ! $no_search_terms )
		{
			$_BETHEL['SearchTerms'] .= '<b>' . _( 'Address contains' ) . ': </b>' .
