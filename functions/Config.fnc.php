<?php
/**
 * BETHELSIS & Program Configuration functions
 *
 * @package BETHELSIS
 ...
 *
 * @global array  $_BETHEL     Sets $_BETHEL['Config']
...
function Config( $item, $value = null )
{
	...
	if ( ! isset( $_BETHEL['Config'][ (string) $item ] ) )
	{
		...
		$_BETHEL['Config'] = DBGet( "SELECT TITLE,CONFIG_VALUE,SCHOOL_ID
			FROM CONFIG
			WHERE " . $school_where, array(), array( 'TITLE' ) );

		$_BETHEL['Config']['SYEAR'][1]['CONFIG_VALUE'] = $DefaultSyear;
	}

	if ( ! is_null( $value ) )
	{
		if ( ! isset( $_BETHEL['Config'][ (string) $item ][1]['TITLE'] ) )
		{
		...
		elseif ( $value != DBEscapeString( $_BETHEL['Config'][ (string) $item ][1]['CONFIG_VALUE'] ) )
		{
			// Update value (different from current value).
			DBQuery( "UPDATE CONFIG
				SET CONFIG_VALUE='" . $value . "'
				WHERE TITLE='" . $item . "'
				AND SCHOOL_ID='" . $_BETHEL['Config'][ (string) $item ][1]['SCHOOL_ID'] . "'" );
		}
		...
		$_BETHEL['Config'][ (string) $item ][1]['CONFIG_VALUE'] = $value;
	}

	return issetVal( $_BETHEL['Config'][ (string) $item ][1]['CONFIG_VALUE'] );
}
 ...
 * @since 6.0 Handle single quotes in $value with DBEscapeString().
 *
 * @global array        $_BETHEL Sets $_BETHEL['ProgramConfig']
 *
...
function ProgramConfig( $program, $item = 'all', $value = null )
{
	...
	if ( ! isset( $_BETHEL['ProgramConfig'][ (string) $program ] ) )
	{
		$_BETHEL['ProgramConfig'] = DBGet( "SELECT PROGRAM,TITLE,VALUE
			FROM PROGRAM_CONFIG
	...
	if ( ! is_null( $value )
		&& $item !== 'all' )
	{
		if ( ! isset( $_BETHEL['ProgramConfig'][ (string) $program ][ (string) $item ][1]['TITLE'] ) )
		{
			// Insert value (does not exist).
			...
			$_BETHEL['ProgramConfig'][ (string) $program ][ (string) $item ][1]['TITLE'] = $item;
		}
		elseif ( $value != DBEscapeString( $_BETHEL['ProgramConfig'][ (string) $program ][ (string) $item ][1]['VALUE'] ) )
		{
			// Update value (different from current value).
		...
		$_BETHEL['ProgramConfig'][ (string) $program ][ (string) $item ][1]['VALUE'] = $value;
	}

	if ( $item === 'all' )
	{
		return issetVal( $_BETHEL['ProgramConfig'][ (string) $program ] );
	}

	return issetVal( $_BETHEL['ProgramConfig'][ (string) $program ][ (string) $item ][1]['VALUE'] );
}
