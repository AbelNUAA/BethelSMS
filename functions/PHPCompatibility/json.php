...
if ( !function_exists('json_encode') ) {
	function json_encode( $string ) {
		...
		if ( ! ( $BETHELJSON instanceof Services_JSON ) ) {
			require_once 'classes/PHPCompatibility/Services_JSON.php';
			$BETHELJSON = new Services_JSON();
		}

		return $BETHELJSON->encodeUnsafe( $string );
	}
}

if ( !function_exists('json_decode') ) {
	/**
	 * @global Services_JSON $BETHELJSON
	 * @param string $string
