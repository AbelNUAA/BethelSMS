 ...
 *
 * @global array  $_BETHEL Sets $_BETHEL['GetMP']
 *
...
function GetMP( $mp_id, $column = 'TITLE' )
{
	...
	if ( ! isset( $_BETHEL['GetMP'] ) )
	{
		$_BETHEL['GetMP'] = DBGet( "SELECT MARKING_PERIOD_ID,TITLE,POST_START_DATE,
	...
	return empty( $_BETHEL['GetMP'][ $mp_id ][1][ $column ] ) ?
		'' :
		$_BETHEL['GetMP'][ $mp_id ][1][ $column ];
}
