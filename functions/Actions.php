 ...
 *
 * Actions are the hooks that the BETHELSIS core launches at specific points
 * during execution, or when specific events occur. Plugins can specify that
 * one or more of its PHP functions are executed at these points, using the
 * Action API.
 *
 * @global array    $BETHELActions
 *
...
{
	global $BETHELActions;

	// Check if function exists.
	if ( function_exists( (string) $function_to_add ) )
	{
		$BETHELActions[ $tag ][ $priority ][ $function_to_add ] = (int) $accepted_args;
	}
...
function remove_action( $tag, $function_to_remove ) {
	...
	// Check if tag exists.
	if ( array_key_exists( (string) $tag, $BETHELActions ) )
	{
		foreach ( $BETHELActions[ $tag ] as $priority => $actions )
		{
			// Check if function previously added.
			if ( array_key_exists( (string) $function_to_remove, $actions ) )
			{
				unset( $BETHELActions[ $tag ][ $priority ][ $function_to_remove ] );
 ...
 *
 * @return null   Will return null if $tag does not exist in $BETHELActions array
 */
function do_action( $tag, $arg = '' )
{
	...
	if ( array_key_exists( (string) $tag, $BETHELActions ) )
	{
		...
		ksort( $BETHELActions[ $tag ] );

		foreach ( (array) $BETHELActions[ $tag ] as $functions )
		{
