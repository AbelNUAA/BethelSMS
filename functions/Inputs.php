 ...
 *       if ( AllowEdit() && !isset( $_REQUEST['_BETHEL_PDF'] ) && $value != '' && $div )
 *
 ...
 *
 * @global $BETHELLocales Returns simple TextInput() if only 1 locale set
...
{
	global $BETHELLocales,
		$locale;

	$value = is_array( $value ) ? $value[0] : $value;

	if ( count( $BETHELLocales ) < 2 )
	{
...
<?php 	$return = ob_get_clean();
		...
		foreach ( (array) $BETHELLocales as $key => $loc )
		{
...
function MarkDownInputPreview( $input_id )
{
	...
	<div class="md-preview">
		...
		<a href="https://gitlab.com/francoisjacquet/BETHELsis/wikis/Markdown-Cheatsheet" title="<?php echo htmlspecialchars( _( 'Mastering MarkDown' ), ENT_QUOTES ); ?>" target="_blank" class="md-link">
 ...
 * @uses GetInputID() to generate ID from name
 * @uses InputDivOnclick()
 *       if ( AllowEdit() && !isset( $_REQUEST['_BETHEL_PDF'] ) && ! $new && $div )
 *
 * @param  string  $value   Input value.
 * @param  string  $name    Input name.
 * @param  string  $title   Input title (optional). Defaults to ''.
 ...
 * @uses ParseMLField() to get localized options
 *
 * @global $BETHELLocales Returns simple SelectInput() if only 1 locale set
 * @global $locale         Get current locale
 *
 * @param  string         $value    Input value.
 * @param  string         $name     Input name.
