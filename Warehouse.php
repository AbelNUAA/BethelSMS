...
if ( ! defined( 'BETHEL_DEBUG' ) )
{
	// Debug mode (for developers): enables notices.
	define( 'BETHEL_DEBUG', false );
}

error_reporting( BETHEL_DEBUG ? E_ALL : E_ALL ^ E_NOTICE );
...
if ( ! empty( $_GET['locale'] )
	&& in_array( $_GET['locale'], $BETHELLocales ) )
{
	$_SESSION['locale'] = $_GET['locale'];
}
elseif ( empty( $_SESSION['locale'] ) )
{
	$_SESSION['locale'] = $BETHELLocales[0]; // English?
}
...
bindtextdomain( 'BETHELsis', $LocalePath );

// Ensures text returned is utf-8, quite often this is iso-8859-1 by default.
bind_textdomain_codeset( 'BETHELsis', 'UTF-8' );

// Sets the domain name, this means gettext will be looking for a file called BETHELsis.mo.
textdomain( 'BETHELsis' );
 ...
 * @since 2.9
 */
// Check if version in DB < BETHEL_VERSION.

if ( version_compare( Config( 'VERSION' ), BETHEL_VERSION, '<' ) )
{
 ...
 * Modules
 *
 * Core modules (packaged with BETHELSIS): cannot be deleted.
 */
...
$BETHELModules = unserialize( Config( 'MODULES' ) );
 ...
 * Plugins
 *
 * Core plugins (packaged with BETHELSIS): cannot be deleted.
 */
$BETHELCorePlugins = array(
	'Moodle',
);

$BETHELPlugins = unserialize( Config( 'PLUGINS' ) );

_LoadAddons( $BETHELPlugins, 'plugins/' );
 ...
 *
 * @global $_BETHEL  Uses $_BETHEL['ProgramLoaded']
 *
 * @uses isPopup()
 * @uses isAJAX()
 * @uses ETagCache()
...
<head>
	...
	<link rel="stylesheet" href="assets/themes/<?php echo Preferences( 'THEME' ); ?>/stylesheet.css?v=<?php echo BETHEL_VERSION; ?>" />
	<style>.highlight,.highlight-hover:hover{background-color:<?php echo Preferences( 'HIGHLIGHT' ); ?> !important;}</style>
	<?php

			if ( $_BETHEL['page'] === 'modules'
				|| $_BETHEL['page'] === 'first-login'
				|| $_BETHEL['page'] === 'create-account'
				|| $_BETHEL['page'] === 'password-reset' )
			{
				// @since 6.0 Warehouse Header Javascripts.
				WarehouseHeaderJS();
			}
...
<body class="<?php echo $_BETHEL['page']; ?>">
<?php
			if ( $_BETHEL['page'] === 'modules' ):
...
<?php
			...
			if ( isset( $_BETHEL['page'] )
				&& $_BETHEL['page'] === 'modules' ): ?>
<script>
	var modname = "<?php echo issetVal( $_BETHEL['ProgramLoaded'], '' ); ?>";
...
function WarehouseHeaderJS()
{
	...
	<script src="assets/js/plugins.min.js?v=<?php echo BETHEL_VERSION; ?>"></script>
